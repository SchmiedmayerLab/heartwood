...
The final image carries `README.md`, `ACRONYMS.md`, `docs/`, and `design/` under `/opt/heartwood`, including the Terra-style Jupyter demo notebook. The generic image remains the portable runtime baseline; Terra users should select the separate `edge-terra` or `edge-terra-smoke` platform image after the main-branch publish workflow creates those tags.
