...
run_heartwood --session-id "${session_id}" \
  --prompt "Call the terminal tool to execute this exact command: ${heartwood_python} ${runtime_root}/vendor/heartwood-skills/skills/omop-cohort-summary/scripts/run.py --data-root input --target-condition-concept-id 201826 --minimum-age 18 --aggregate-count-floor 20 --output cohort-summary.json && printf %s heartwood-agent-exact-ok > ${exact_name} && cat cohort-summary.json. The marker file must contain exactly heartwood-agent-exact-ok with no trailing newline. Do not describe the command as text and do not call another tool after it completes. Wait for the terminal result, then report the aggregate cohort result." \
  | tee -a "${transcript}"
