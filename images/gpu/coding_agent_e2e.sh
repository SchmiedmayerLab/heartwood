...
reasoning_parser="$(jq -er '.configuration.reasoning_parser // ""' <<<"${configuration}")"
enforce_eager="$(jq -er 'if .configuration.enforce_eager then "1" else "0" end' <<<"${configuration}")"
...
export HEARTWOOD_VLLM_REASONING_PARSER="${reasoning_parser}"
export HEARTWOOD_VLLM_ENFORCE_EAGER="${enforce_eager}"
