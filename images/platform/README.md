...
Use `images/platforms.toml` as the source of truth for platform base image, home directory, user, Jupyter prefix, supported architectures, tag names, and required evidence. Do not add a new platform image target without adding a manifest entry, Bake target, static tests, and documentation for the platform-specific validation evidence. The contributor-facing mechanism is documented in `docs/platform-images.md`. Published targets keep cache, SBOM, and provenance settings; local-only CI `--load` targets use the Docker driver when they depend on a locally tagged base image and disable attestations because Docker's local exporter cannot load attested image indexes.
