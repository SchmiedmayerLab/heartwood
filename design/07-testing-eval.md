...
1. **Pure-code tests.** Detector evidence, endpoint normalization, deployment policy, credentials, action settings, upstream analyzer composition, audit chaining and scrubbing, adapters, model connections, provider catalogs, model settings, artifact integrity, and Skill metadata are tested without a model call.
...
- **OpenHands compatibility.** Every SDK or tools upgrade must pass real event translation, action confirmation, rejection, automatic low-risk execution, restoration of every unresolved action, sequential review of multi-action confirmation stops without implicit sibling approval, persistence and resume at each review position, native Skill loading, terminal and file execution, and offline container integration. Until sequential review is implemented, the existing fail-closed batch-rejection test remains a limitation check rather than acceptance evidence.
