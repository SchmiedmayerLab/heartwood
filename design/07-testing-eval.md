...
4. **Interface contracts.** CLI, notebook, REST, WebSocket, Server-Sent Events, and web view models consume the same gateway services and session events. Cross-interface tests change project model and action settings through one surface, restart the gateway, and require the other surfaces to observe the persisted state. Vitest, Testing Library, and Playwright cover first-run readiness, provider-source preparation, CPU and GPU reviewed-model presentation, download progress, compute gating, focus and settings refresh, grouped action review, and desktop and narrow Jupyter layouts. Packaged-gateway and Jupyter-proxy tests cover deployment transports. A real-browser system test uses the built interface, live gateway, OpenHands SDK, loopback model fixture, terminal tool, session-localized inputs, all three reference Skills, repeated action approval, successful and failed tool outcomes, persisted aggregate artifacts, audit download, and CLI replay of the browser-created session.
