...
5. **Interface contract tests.** CLI commands, interactive-session transcripts, notebook view models, and web-UI view models are generated from the same synthetic session events and compared against stable snapshots; the gateway's translation between the agent-server event stream and the session contract is covered by contract tests, and the WebSocket/SSE transport is exercised against a fake agent-server. Web-UI components are tested with Vitest and Testing Library.
