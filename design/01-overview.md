...
- Not a new agent framework or agent loop — it builds on the OpenHands SDK.
...
- **Analyst-researcher (primary, often non-technical).** Gets a guided, low-configuration experience: detection proposes a vetted workflow, they confirm in plain language, and they never need to read code to stay safe. The outward-facing experience can be a notebook widget surface in Terra, backed by the same session model as the CLI.
...
A researcher launches the image on their platform and starts a session from the CLI or a notebook widget. In one guided flow, they perform **reproducible in-perimeter cohort extraction → QC → a baseline model**, producing an **egress-attestation report** suitable for an IRB. This is the canonical end-to-end capability the platform delivers.
