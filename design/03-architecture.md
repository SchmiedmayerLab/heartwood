...
Local profiles use the same contract. The local connection lists models reported by an active Ollama, vLLM, SGLang, llama.cpp, or other OpenAI-compatible runtime and presents reviewed single-file artifacts and model snapshots through one download manager. The image may include a portable llama.cpp server binary or an explicit GPU runtime, but model weights live in project storage or a platform disk and are selected explicitly. The reviewed catalogs are guidance and reproducible download metadata, not weights embedded in an image. Background downloads report transferred and expected bytes through the gateway; successful verification records the local selection and standard loopback model profile in shared project configuration. The web UI polls only while a transfer is active, then reloads shared project state. Download completion produces `compute-required`, not runtime readiness or a model-capability claim, until `heartwood launch` starts and verifies the selected server.
...
- Skills, activity and audit, and project settings are secondary views. Repository-verified Skills and installed extensions use the same trust language as the Skill contract. An unconfigured project opens settings automatically. The readiness summary projects the same content-free checks as `heartwood doctor`; model setup lists active local services, reviewed CPU and GPU downloads, platform-provided research connections, and user-configurable providers; it loads exact model identifiers from the selected source, reports local-model transfer progress, gates the composer on runtime readiness, and places complete profile fields under advanced options. Action-confirmation settings are a separate tab. Platform policy is read-only in the researcher interface.
...
Every interactive state is projected from the gateway contract. The browser does not synthesize action outcomes, workflow completion, project readiness, policy decisions, Skill verification, model capability, or audit integrity. Model-source choices, connections, normalized catalog entries, reviewed local models, download status, and readiness checks are gateway-supplied data rather than vendor-specific interface code.
...
Notebook support reuses the gateway and web UI through the platform's authenticated proxy. Its Python adapter can inspect the same readiness, model source, reviewed downloads, action settings, sessions, and events, while notebook widgets remain a compact status and launch bridge rather than a third full interaction design.
