...
- Pending OpenHands confirmations appear inline near the conversation as one action-set review with every proposed tool, upstream risk, summary, and exact structured arguments supplied by OpenHands, including command, path, content, scope, or network intent when present. The private session stream and every transcript projection retain those arguments after resolution for replay, while the audit layer scrubs the complete argument payload. Missing metadata is omitted rather than inferred. **Allow all once** and **Reject all** match the upstream set-level decision; no interface implies that one member can execute selectively.
...
The installation root and project root are separate sibling directories on the same approved durable storage. The installation root may serve several projects and remains outside every project's agent working directory, while each project exclusively owns its files and `.heartwood/` state. The native installer uses its current directory as the installation root unless `--root` explicitly selects another path. During installation, it overrides package-manager homes, caches, configuration, and temporary paths with restrictive locations under `<installation-root>/.installer/`; it must not consume a researcher's home quota or write retained installation state to a system temporary directory. A successful installation removes that transient installer directory, while an interrupted installation may retain it under the installation root so the same command can resume without using storage outside the approved boundary. A platform-specific installation records the selected platform in its stable command launcher so later commands do not depend on a researcher exporting a platform environment variable.

Native runtime environments use the persistent bootstrap interpreter under their versioned runtime root and prohibit package-manager interpreter downloads. Both their executable links and `pyvenv.cfg` homes must remain valid after `<installation-root>/.installer/` is removed.
