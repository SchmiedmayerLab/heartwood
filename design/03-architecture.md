...
- Pending OpenHands confirmations appear inline near the conversation with the proposed tool, upstream risk, summary, and any structured scope or network intent supplied by OpenHands. Missing metadata is omitted rather than inferred. **Allow once** and **Reject** remain the only per-action decisions. The current adapter rejects a multi-action upstream stop before execution; the required sequential-review contract presents each action separately and starts execution only after the pending set has the required decisions.
