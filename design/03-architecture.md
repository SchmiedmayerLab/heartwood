...
- Skills, activity and audit, and model setup are secondary views. Repository-verified Skills and installed extensions use the same trust language as the Skill contract. Model setup leads with active-route status and validation; provider fields and local artifact management use progressive disclosure. Platform policy is read-only in the researcher interface.
...
Session state and downloaded model artifacts live on the workspace or platform disk so they survive container replacement and platform autopause. No external Heartwood database is required for the single-user interactive deployment.
