...
- **TypeScript** — used in Phase 0 for the researcher web UI, built on the Stanford Spezi web stack: `@stanfordspezi/spezi-web-design-system` (components) and `@stanfordspezi/spezi-web-configurations` (shared ESLint/Prettier/TypeScript configs), with Vite, Vitest, Testing Library, and Playwright, aligned with Spezi template conventions. Strong linting is required and non-negotiable: ESLint + Prettier and strict `tsc` via the shared Spezi configs, run in CI. A standalone single-page app is preferred over a JupyterLab extension so one build artifact stays portable across platforms.
...
- **TypeScript:** run ESLint, Prettier, strict `tsc`, Vitest with coverage, npm audit, npm license checks, asset build, and Playwright browser smoke tests over `packages/webui` through the `Web UI` workflow.
- **Containers:** repo-local container smoke tests run Dockerfile and Bake-target checks, then run the smoke image flavor with runtime network disabled on native GitHub-hosted `linux/amd64` and `linux/arm64` runners. The Terra platform smoke path builds a lightweight Terra-compatible CI base and then runs the same platform Dockerfile with runtime network disabled on `linux/amd64`. The main-branch publish workflow builds each generic architecture on its native runner with SBOM and provenance attestations, merges the architecture images into multi-architecture GHCR `edge`, `edge-coder-7b`, `edge-smoke`, and `edge-providers` manifests where the dependency stack supports both platforms, publishes `edge-terra`, `edge-terra-coder-7b`, and `edge-terra-smoke` from the real Terra base on `linux/amd64` as Docker schema-2 manifests without Buildx attestations for Leonardo compatibility, verifies every user-facing published tag through unauthenticated registry inspection, and runs the real-base Jupyter contract and launch smokes against the published `edge-terra-smoke` image after clearing Docker build cache so the constrained runner does not also unpack the 7B runtime layer. Platform-derived image publication uses `images/platforms.toml` plus `images/platform/scripts/verify_registry_manifest.py` so each platform declares its manifest media type, config media type, architecture set, and non-platform manifest policy. Optional GPU runtime profiles require separate image/runtime choices and GPU-capable CI rather than being part of the portable CPU baseline.
