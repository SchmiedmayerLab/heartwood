...
13. **Carina and NVIDIA execution are only partially live-exercised.** Release `0.1.0` installed and verified a reviewed public model on a real Carina L40S allocation, and a local model produced genuine OpenHands action proposals. The run required manual FFmpeg installation, FlashInfer sampler override, partition selection, and platform-profile correction; the action workflow then failed on confirmation identifiers and multi-action rejection. A native container GPU run, live Stanford gateway request, and complete synthetic acceptance remain unrecorded. See [Carina Release 0.1.0 Validation Findings](../docs/carina-live-validation.md).
