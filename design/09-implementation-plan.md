...
- The Terra-derived notebook image flavors publish as `edge-terra`, model-explicit alias `edge-terra-coder-7b`, and `edge-terra-smoke`, derive from `us.gcr.io/broad-dsp-gcr-public/terra-jupyter-python:1.1.6`, install a managed Python 3.12 Heartwood runtime under `/opt/heartwood`, register a Heartwood Jupyter kernel under `/opt/conda`, keep platform Python and Jupyter ahead of the Heartwood virtual environment, preserve `/home/jupyter`, port `8000`, the Terra base image entrypoint, and the Leonardo-style `run-jupyter.sh` path, bundle the 7B model in the default Terra runtime image, and publish user-facing Terra tags as single-platform Docker schema-2 manifests for Leonardo compatibility.
...
- The container smoke workflow runs the offline stack smoke on native GitHub-hosted `linux/amd64` and `linux/arm64` runners for pull requests, pushes to `main`, and manual dispatch. It also builds a lightweight Terra-compatible CI base with real Jupyter notebook packages and Terra-style config, builds `edge-terra-smoke-ci` through the same platform Dockerfile on `linux/amd64`, runs the Terra Jupyter contract smoke, runs the Terra platform image smoke, runs the full offline stack smoke with runtime network disabled, launches the inherited Jupyter entrypoint at `/notebooks/`, and launches the Leonardo-style `run-jupyter.sh` path at `/notebooks/<project>/<cluster>/`.
...
- `images/platform/terra-ci-base.Dockerfile` defines a lightweight Terra-compatible notebook base for pull-request smoke tests; it is not a user-facing Terra image and exists to test the platform Dockerfile, Jupyter home/prefix assumptions, real Jupyter launch behavior, kernel registration, packaged UI, and offline stack without pulling the real Terra base on every pull request.
- `images/platform/scripts/terra_jupyter_contract_smoke.sh` verifies that the Terra image keeps the platform user, `/home/jupyter` working directory, Jupyter port, Jupyter binaries, `run-jupyter.sh`, platform Python precedence, Heartwood kernel, and notebook API contract intact.
- `images/platform/scripts/terra_jupyter_launch_smoke.sh` starts the image through the inherited Jupyter entrypoint and the Leonardo-style `run-jupyter.sh` path, then verifies the expected `/notebooks/...` routes and root Jupyter 404 behavior.
...
- Terra image tests statically verify the selected Terra base image, `/home/jupyter` assumptions, packaged Heartwood docs/notebook, loopback web UI port, Jupyter service path, Jupyter kernel registration, publication tags, inherited Jupyter entrypoint, image working directory, notebook port, launch-critical environment settings, and absence of baked provider secrets.
- Terra pull-request smoke tests launch the platform image through its inherited Jupyter entrypoint and through the Leonardo-style `run-jupyter.sh` path, verify that `/notebooks/` and `/notebooks/<project>/<cluster>/` are reachable as applicable, and verify that `/` returns the expected Jupyter 404.
