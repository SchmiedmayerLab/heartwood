...
- Published images contain a CPU llama.cpp runtime and reviewed Hugging Face artifact metadata but no model weights. The catalog supports digest-pinned single-file llama.cpp artifacts and revision-pinned multi-file snapshots for the native vLLM path. Downloads are explicit, storage-checked, atomically published, stored outside image layers, and covered by an exact local checksum manifest. The catalog separates a tool-capable agent demonstration artifact from a coding-output experiment rather than assuming that coding text quality implies OpenHands tool compatibility.
...
- Explicit generic and Terra NVIDIA targets install the artifact-hash-locked vLLM runtime in an isolated cache boundary while retaining the same Heartwood payload and no-weight policy. Pull requests build both filesystems through dedicated BuildKit validation stages that keep uv archives out of final filesystems, import TorchCodec and vLLM against packaged FFmpeg, and reject embedded model artifacts except for the exact Hadamard transform tensor supplied by the hash-locked `compressed-tensors` dependency, then use the cache-only exporter instead of serializing complete images into the runner. Main pushes by digest, pulls and runs the shared verifier against each exact candidate, creates digest-checked immutable tags, and promotes both moving channels only after both candidates pass. The portable images remain the defaults. A native Carina deployment uses a version-pinned Micromamba bootstrap, the application lock, and the same hash-locked vLLM requirements in separate environments; disables the optional FlashInfer sampler path that requires an undeclared CUDA compiler; requires exact snapshot-manifest coverage; discovers an available GPU partition; stages into fresh job scratch with progress; and binds inference to loopback.
...
13. **The corrected Carina native path requires clean release validation.** Release `0.1.0` installed on Carina and a reviewed public model produced genuine OpenHands action proposals on an L40S allocation. Constrained execution, rejection, scrubbed audit export, and fresh-process replay passed after manual runtime and configuration corrections. Release `0.1.1` packages those corrections and tests the installation, dependency, partition, setup-context, progress, grouped-confirmation, replay, and audit contracts, but the unmodified published artifact has not completed the full acceptance run. [Issue #25](https://github.com/SchmiedmayerLab/heartwood/issues/25) owns the clean rerun.
14. **NVIDIA execution evidence remains resource-qualified.** Pull requests build the generic and Terra NVIDIA targets and validate their dependency imports and runtime contracts, but standard GitHub-hosted jobs do not load a model on a native NVIDIA device. Real vLLM model loading, warm-up, OpenHands tool calling, and shutdown still require the opt-in capable-model workflow or an approved platform run.

## Priority 1 — Release-Candidate Runtime Contract

**Objective:** complete the remaining runtime-identity, persistence, ingress, and live-platform gates without adding another agent loop, session owner, or platform-specific interaction model.

### Deliverables

Complete the following passes in order; each pass must meet the shared exit criteria before release-candidate promotion.

#### Pass 1 — Runtime Identity And Persistence
...
#### Pass 2 — Ingress And Credential Boundaries
...
#### Pass 3 — OpenHands Event And Control Alignment

- [ ] Complete [Issue #24](https://github.com/SchmiedmayerLab/heartwood/issues/24): project typed, durable OpenHands model, tool, action-set, plan, lifecycle, failure, and token events; preserve ordering and idempotent restart; and make the CLI, web UI, and notebook bridge consume that projection.
- [ ] Add gateway-owned cancellation backed by a supported OpenHands pause or conversation-control operation. Until an in-flight turn is cancellable, every interface must distinguish pausing an idle session from requesting interruption of active work.
- [ ] Add a remote-gateway CLI transport behind the existing interaction controller and command/event protocol. Validate reconnect, replay cursor, grouped action-set decisions, and audit export without importing OpenHands CLI state or creating a second conversation owner.

#### Pass 4 — Carina And NVIDIA Acceptance

- [ ] Publish release `0.1.1` with the packaged FFmpeg, sampler, snapshot-download, partition-discovery, setup-context, progress, and grouped-confirmation corrections; complete the clean synthetic Carina rerun in [Issue #25](https://github.com/SchmiedmayerLab/heartwood/issues/25) without manual environment edits or executable wrappers.
- [ ] On a native NVIDIA runner or approved platform allocation, verify vLLM import, model loading, warm-up, loopback readiness, genuine OpenHands multi-action proposal, grouped approval and rejection, tool execution, restart replay, scrubbed audit export, runtime shutdown, and scratch cleanup.
- [ ] Run one opt-in, content-free Stanford AI API Gateway catalog and tool-call test under an approved credential and retain only route, status, model-identifier, action, and scrubbing evidence.
- [ ] Evaluate the current Terra Jupyter Python base options. Change the pinned base only when a candidate passes the complete user, kernel, entrypoint, Leonardo route, proxy, storage, model-runtime, and publication contract.
...
- A terminal user receives ordered incremental model and tool activity, can reconnect without duplicate events, reviews every pending OpenHands action as one clearly grouped set, and can request a supported interruption with an explicit resulting lifecycle state.
...
- An unmodified published native artifact completes the documented Carina synthetic workflow without installing runtime dependencies, creating executable wrappers, editing settings through Python, discovering an undocumented partition override, or entering internal confirmation identifiers.
- A capable terminal presents every member of a pending OpenHands action set in one keyboard-navigable review panel, survives resize and narrow SSH dimensions, and produces the same whole-set decisions, execution outcomes, replay, and audit records as line mode and the web UI.
...
- [ ] Complete [Issue #26](https://github.com/SchmiedmayerLab/heartwood/issues/26): preserve typed OpenHands action details, add a bounded read-only workspace and change-review API, and project exact commands, affected paths, files, and diffs through the shared CLI and web contracts without copying that sensitive session evidence into scrubbed audit exports.
...
- A researcher can create or resume a session, configure or select an authorized model route, submit a task, follow model and tool activity, allow or reject a clearly grouped action set, pause or resume, and export the audit record without reading operator documentation.
...
- [ ] Publish the canonical README, operational tutorials, design set, API reference, image reference, security model, status matrix, and limitations through GitHub Pages without maintaining copied Markdown. Pull requests must build the site strictly, `main` must update a clearly labeled development view, and each published release must add an immutable Semantic Version snapshot and advance a `latest` alias without modifying older snapshots. Gate publication on internal-link, external-link, accessibility, and tutorial-command checks, and use a dedicated Pages deployment environment with serialized publication.
