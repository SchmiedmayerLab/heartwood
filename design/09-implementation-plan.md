...
- The Terra-derived notebook image flavors publish as `edge-terra` and `edge-terra-smoke`, derive from `us.gcr.io/broad-dsp-gcr-public/terra-jupyter-python:1.1.6`, install a managed Python 3.12 Heartwood runtime under `/opt/heartwood`, register a Heartwood Jupyter kernel under `/opt/conda`, preserve the Terra base image entrypoint, and publish user-facing Terra tags as single-platform Docker schema-2 manifests for Leonardo compatibility.
...
- The container image publish workflow builds `docker-bake.hcl` targets on native `linux/amd64` and `linux/arm64` runners on `main`, merges the generic architecture outputs into multi-architecture manifests, frees runner disk before the real Terra build, publishes `edge`, `edge-smoke`, `edge-providers`, `edge-terra`, `edge-terra-smoke`, and commit-SHA flavor tags to GitHub Container Registry, verifies every user-facing public tag through unauthenticated registry inspection, verifies generic tags resolve to both baseline Linux architectures, and verifies Terra tags return `application/vnd.docker.distribution.manifest.v2+json` `linux/amd64` image manifests rather than OCI indexes until the selected Terra base and Leonardo support a broader manifest shape.
...
- Keep post-publish registry verification for every public image tag: generic unified tags must resolve to `linux/amd64` and `linux/arm64`, while platform-derived tags must match the platform manifest's supported architecture set and manifest media type.
...
- Image publication checks inspect public unified tags with an unauthenticated Docker config after manifest creation and fail if a tag is private, any required platform is missing, or any platform-specific tag publishes an unsupported architecture.
