...
13. **Carina and NVIDIA execution are only partially live-exercised.** Release `0.1.0` installed and verified a reviewed public model on a real Carina L40S allocation; a local model produced genuine OpenHands action proposals; and constrained approve, execute, reject, scrubbed audit, and fresh-process replay checks passed. The run required manual FFmpeg installation, FlashInfer sampler override, partition selection, and platform-profile correction, and a normal multi-action turn failed on confirmation identifiers and batch rejection. A clean-artifact runtime and conversation restart, native container GPU run, live Stanford gateway request, and complete synthetic acceptance remain unrecorded. See [Carina Release 0.1.0 Validation Findings](../docs/carina-live-validation.md).
14. **Native release and launch are not live-validated.** Bundle construction, checksum enforcement, versioned installation, command publication, platform detection, scheduler consent, local-runtime supervision, and cleanup are automated, and the tagged `0.1.0` artifact was partially exercised on Carina. The published artifact did not complete startup and action execution without manual intervention, and no approved Terra native environment has completed the path.
...
- [ ] Complete the native-runtime pass identified by the `0.1.0` Carina exercise: lock and package FFmpeg and TorchCodec with vLLM, select a supported sampler that does not require an undeclared CUDA compiler, validate imports separately from allocation-side startup, and apply the same resolved dependency contract to the native bundle and NVIDIA images.
- [ ] Complete the Carina launch pass identified by the `0.1.0` exercise: validate persistent capacity and required roots, discover or deployment-configure valid Slurm partitions, preserve typed platform and allocation context across subprocesses, verify job scratch, and display the managed agent workspace before accepting a task.
- [ ] Add installation and launch progress for dependency resolution, model verification, scratch staging, runtime import, model loading, warm-up, endpoint readiness, setup, and conversation startup. Long stages must report elapsed time; failures must include the stage, process exit status, concise sanitized root cause, log location, and next action.
- [ ] Replace internal confirmation identifiers and fail-closed multi-action rejection with a persisted sequential-review contract over each OpenHands pending set. Present one action at a time with tool, summary, scope when supplied, and upstream risk; accept `/allow` and `/reject` for the displayed action; execute only after all required decisions permit the pending set; define rejection and restart semantics; and record every decision and execution outcome without implicitly approving a sibling action.
- [ ] Upgrade the Textual client into the primary interactive terminal review surface: add arrow-key navigation, visible focus, color with non-color equivalents, **Allow once**, **Reject**, and details controls, pending-action position, wrapped narrow-terminal output, and explicit working and waiting states. Keep slash commands for line mode and automation, suppress the just-entered message and routine allowed-route internals from the live response, and retain the complete events for replay and audit.
- [ ] Add regression gates for the complete Carina findings: native dependency imports, real NVIDIA vLLM startup and warm-up, setup-context preservation, partition validation, progress and root-cause projection, sequential multi-action review and restart, narrow SSH terminal interaction, genuine model response, executed and rejected synthetic actions, replay, and scrubbed audit export.
...
- A capable terminal presents each pending action as a keyboard-navigable review step, survives resize and narrow SSH dimensions, and produces the same decisions, execution outcomes, replay, and audit records as line mode and the web UI.
