...
- [ ] Add a Stanford AI API Gateway platform connection and policy template using the documented provider-compatible base URL, exact upstream model aliases, `/v1/models` discovery, external credential reference, and required compatibility behavior. Validate the OpenHands and LiteLLM request headers and caching options against the gateway rather than reading Claude settings or assuming Claude-specific environment flags apply. Add mocked catalog, routing, secret-scrubbing, denied-policy, and header-compatibility tests; keep live tests opt-in and content-free. Record data-classification eligibility only from the current Stanford GenAI Evaluation Matrix and applicable project approval.
