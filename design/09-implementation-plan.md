...
13. **The corrected Carina native path requires clean release validation.** Release `0.1.0` installed on Carina and a reviewed public model produced genuine OpenHands action proposals on an L40S allocation. Constrained execution, rejection, scrubbed audit export, and fresh-process replay passed after manual runtime and configuration corrections. The repository now packages those corrections and tests the installation, dependency, partition, setup-context, progress, grouped-confirmation, replay, and audit contracts, but an unmodified release containing the fixes has not completed the full acceptance run. [Issue #25](https://github.com/SchmiedmayerLab/heartwood/issues/25) owns the clean published-artifact rerun.
...
- [ ] Publish a native release candidate containing the packaged FFmpeg, sampler, snapshot-download, partition-discovery, setup-context, progress, and grouped-confirmation corrections; complete the clean synthetic Carina rerun in [Issue #25](https://github.com/SchmiedmayerLab/heartwood/issues/25) without manual environment edits or executable wrappers.
