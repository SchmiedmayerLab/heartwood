...
Replace the target and tag names when adding a new platform. Keep `--set <target>.platform=<architecture>` on any `--load` Bake command used by a Docker container-driver builder, because Docker's local image exporter does not load manifest lists.
