...
The pull-request Terra smoke job builds `images/platform/terra-ci-base.Dockerfile` and then builds `terra-smoke-ci` through the same `images/platform/Dockerfile`. That job validates Heartwood packaging, the Jupyter home/prefix assumptions, kernel registration, the platform smoke script, and the offline stack with runtime network disabled without pulling the multi-gigabyte Terra base or 7B model on every pull request. The main-branch publish job builds `terra-runtime` and `terra-smoke` from the real Terra base after freeing runner disk space, then verifies the published Terra tags.
