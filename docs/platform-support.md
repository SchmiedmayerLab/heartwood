...
|---|---|---|---|---|
| Generic Linux or Jupyter environment | Implemented | `edge` and `sha-<git-sha>` for `linux/amd64` and `linux/arm64` | Native architecture builds, no-weight checks, real OpenHands loopback integration, action-confirmation modes, local llama.cpp fixture, web UI, notebook proxy, Skills, and audit export | Self-hosted deployments must validate their own identity, storage, network, and data controls. |
