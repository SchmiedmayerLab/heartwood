...
On first use, bare `heartwood` guides model selection and then opens the conversation. Later invocations resume the same project setup. A provider API key entered at the prompt is held only for that process; platform-managed credential bindings can survive a restart without storing a secret in `.heartwood/`.
